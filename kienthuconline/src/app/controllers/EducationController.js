...
class EducationController {
    index (req, res,next) {
        ...
        if(page) {
            ...
            Post.find({field: 'giao-duc' }).sort({createdAt: -1})
            .skip(skipPost)
            .limit(pageSize)
            .then(technology =>
                res.json(technology))
            ...
            if(!req.id) {
            Promise.all([ Post.find({field: 'giao-duc' }),Post.find({field: 'giao-duc' }).sort({count: -1}), Post.find({})])
            .then(values => {
            ...
            .catch(err=> res.status(500).json('Sever lỗi'))
                ...
                Promise.all([ Post.find({field: 'giao-duc' }),Post.find({field: 'giao-duc' }).sort({count: -1}), Post.find({}), User.findOne({id: req.id})])
                .then(values => {
